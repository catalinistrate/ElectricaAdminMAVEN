    ...
    public String ListIssue(Client c){
        ...
        for(int i=0; i<_dataManager.Issues.size(); i++){
        	if(_dataManager.Issues.get(i).Client.equals(c)){
        	    Issue currentIssue = _dataManager.getInvoicesList().get(i);
            	 pen += String.format("Year: %d, Month: %d,ToPay: %2.0f Paid: %2.0f\n",currentIssue.Year,currentIssue.Month,currentIssue.ToPay,currentIssue.Paid);
            	 s += pen;
        	}            
        }  
        return s;
